    ...
    if api_key:
        ...
        if response.status_code == 200:
            ...
            if not data["result"]["rows"]:
                ...
                df["Action"] = df.apply(
                    lambda row: "📈 BUY"
                    if row["token_bought_symbol"] == token_symbol
                    else "📉 SELL",
                    axis=1,
                )
                ...
                df_display = df[
                    ["block_time", "tx_from_ens", "Action", "Amount USD", "tx_hash_url"]
                ].rename(
                    columns={
                ...
                st.dataframe(
                    df_display,
                    column_config={
                        "Transaction Hash (Link)": st.column_config.LinkColumn(
        ...
        st.error(
            "API key is not set. Please set the DUNE_API_KEY environment variable."
        )
